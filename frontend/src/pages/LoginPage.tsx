...
export function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : tx(t.login.error, lang));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-br from-academy-bg via-[#121a28] to-academy-bg">
      ...
      <div className="w-full max-w-md rounded-2xl border border-academy-border bg-academy-panel p-6 sm:p-8 shadow-xl">
        <div className="flex items-center justify-center gap-2 mb-6">
          ...
          <h1 className="text-2xl font-bold">{tx(t.login.title, lang)}</h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block text-sm">
            <span className="text-academy-muted">{tx(t.login.email, lang)}</span>
            <input
              type="email"
              required
              value={email}
          ...
          <label className="block text-sm">
            <span className="text-academy-muted">{tx(t.login.password, lang)}</span>
            <input
              type="password"
              required
              minLength={6}
          ...
          >
            ...
            {tx(t.login.submit, lang)}
          </button>
        </form>
        <p className="mt-4 text-center text-sm text-academy-muted">
          {tx(t.login.noAccount, lang)}{" "}
          <Link to="/register" className="text-academy-accent hover:underline">
            {tx(t.login.register, lang)}
          </Link>
        </p>
      </div>
    </div>
