...
from app.models import Difficulty, Level, ProgressStatus, UserProgress, Track
from app.schemas import LevelDetailOut, LevelMapItem, RunCodeRequest, RunCodeResponse, LevelCreate
...
async def level_map(
    ...
    result = await db.execute(
        ...
        .where(Level.task_text_ru.isnot(None))
        .where(Level.task_text_ru != "")
...
@router.post("/track/{track_id}", response_model=LevelDetailOut)
async def create_level(
    ...
    result = await db.execute(select(func.coalesce(func.max(Level.order_num), 0)).where(Level.track_id == track_id))
    next_order_num = result.scalar_one() + 1
    ...
    level = Level(
        ...
        solution_tests=body.solution_tests,
        allowed_concepts=body.allowed_concepts,
        theory_ru=body.theory_ru,
        theory_en=body.theory_en,
        theory_kz=body.theory_kz,
    )
    db.add(level)
    await db.commit()
    await db.refresh(level)
    
    result = await db.execute(select(Level).options(selectinload(Level.difficulty)).where(Level.id == level.id))
